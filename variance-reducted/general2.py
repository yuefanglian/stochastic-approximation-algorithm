            ...
            for j in range(n):
                if j < t or j > t:
                    grad[t] = grad[t] - 2 * self.M[t][j] * (1 - self.p) ** x[j] * (1 - self.p) ** x[t] * math.log(
                        1 - self.p) + self.M[j][t] * (1 - self.p) ** x[t] * math.log(1 - self.p)
        return grad + noise
...
if __name__ == "__main__":
    ...
    for k in range(K):
        ...
        plt.legend()
        plt.savefig("D:/2023文件/实验图像/20230309/general2/temp{}.png".format(k))  # 输入地址，并利用format函数修改图片名称
    ...
    plt.legend()
    plt.savefig("D:/2023文件/实验图像/20230309/general2/temp{average}.png")  # 输入地址，并利用format函数修改图片名称
    plt.show()
