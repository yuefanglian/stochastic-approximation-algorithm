...
class Submodular_QP:
    ...
    def generate(self, n):
        ...
        self.h = -0.2 * self.H.dot(np.ones((n, 1))) # 给定h=-H^{T}u,u=(1,...1)^{T},为了保证函数非单调性
        self.c = float(-0.5* np.ones((n, 1)).T.dot(self.H.dot(np.ones((n, 1)))))
    ...
    def exact_value_oracle(self, x):
        # return f(x)
        y = float(0.5 * x.T.dot(self.H.dot(x)) + self.h.T.dot(x)+self.c)
        return y
...
def projected_gradient_ascent(sub: Submodular_QP, T=500, batch=1):
    np.random.seed(k)
...
#     for i in range(T):
#         outcome[i] = H.exact_value_oracle(point)
#         g = cvxopt.matrix(np.concatenate((np.ones((m, 1)), (np.ones((n, 1)) - point), np.zeros((n, 1))), axis=0))
#         gradient = H.stochastic_gradient_oracle(point)
#         sol = cvxopt.solvers.lp(cvxopt.matrix(-gradient), G, g)  # 针对非单调，需要加约束，刻画约束
...
def spider_stochastic_fw(H, T=500, batch=1):
    np.random.seed(k)
    m = Submodular_QP.constraints.shape[0]
    n = Submodular_QP.constraints.shape[1]
    outcome = [0] * T
    ...
    for i in range(T):
        ...
        g_t = g_t + sum(H.stochastic_gradient_oracle(point) for j in range(batch)) + sum(
            H.stochastic_gradient_oracle(point1) for j in range(batch))
    return outcome


# our method: Hybrid_SPIDER_SFW
def hybrid_spider_sfw(H, T=500, batch=1):
    # global point1
    np.random.seed(k)
    m = Submodular_QP.constraints.shape[0]
    n = Submodular_QP.constraints.shape[1]
    ...
    for i in range(T):
        ...
        point = (1 - alpha) * point + alpha * np.array(sol['x'])
        rho = (i + 1) ** (-1)
        g_t = sum(H.stochastic_gradient_oracle(point) for j in range(batch)) + (1 - rho) * (g_t - sum(
            H.stochastic_gradient_oracle(point1) for j in range(batch)))
    return outcome
...
if __name__ == '__main__':
    ...
    for k in range(K):
        ...
        #
        # outcome1 = np.array(projected_gradient_ascent(sub, T=epoch))
        # outcome2 = continuous_greedy_method(sub, T=epoch)
        # outcome3 = stochastic_greedy_method(sub, T=epoch)
        # outcome4 = stochastic_greedy_pp(sub, T=epoch)
        # outcome5 = boosting_gradient_ascent(sub, T=epoch, batch=1)
        # outcome6 = np.array(boosting_gradient_ascent(sub, T=epoch, batch=100))
        outcome7 = np.array(spider_stochastic_fw(sub, T=epoch, batch=epoch))
        outcome8 = np.array(hybrid_spider_sfw(sub, T=epoch, batch=100))
        outcome11 = np.array(hybrid_spider_sfw(sub, T=epoch, batch=10))
        outcome9 = np.array(fw(sub, T=epoch))
        outcome10 = np.array(fw2(sub, T=epoch))
        # outcome12 = np.array(fw3(sub, T=epoch))
        ...
        # plt.plot(range(len(outcome3)), outcome3, label='SGA')
        ...
        plt.plot(range(len(outcome7)), outcome7, label='spider_FW')
        plt.plot(range(len(outcome8)), outcome8, label='hybrid_spider_FW(100)')
        plt.plot(range(len(outcome11)), outcome11, label='hybrid_spider_FW(10)')
        plt.plot(range(len(outcome9)), outcome9, label='non_monotone_fw1')
        plt.plot(range(len(outcome10)), outcome10, label='non_monotone_fw2')
        # plt.plot(range(len(outcome12)), outcome12, label='non_monotone_fw3')

        plt.legend()
        plt.savefig("D:/2023文件/实验图像/20230702/non-monotone-qp/temp{}.png".format(k))  # 输入地址，并利用format函数修改图片名称
        plt.clf()  # 需要重新更新画布，否则会出现同一张画布上绘制多张图片

        # outcome1_K = outcome1_K + outcome1
        # outcome6_K = outcome6_K + outcome6
        outcome7_K = outcome7_K + outcome7
        outcome8_K = outcome8_K + outcome8
        outcome11_K = outcome11_K + outcome11
        outcome9_K = outcome9_K + outcome9
        outcome10_K = outcome10_K + outcome10
        # outcome12_K = outcome12_K + outcome12

    # my_font = font_manager.FontProperties(fname=
    #                                       "C:/Windows/Fonts/msyh.ttc")
    plt.xlabel('Iteration Index')
    plt.ylabel('Objective value')
    # plt.plot(range(len(outcome1_K)), outcome1_K / K, label='PGA')
    # plt.plot(range(len(outcome2)), outcome2, label='CG')
    # plt.plot(range(len(outcome3)), outcome3, label='SGA')
    # plt.plot(range(len(outcome4)), outcome4, label='SG++')
    # plt.plot(range(len(outcome5)), outcome5, label='BGA(1)')
    # plt.plot(range(len(outcome6_K)), outcome6_K / K, label='BGA(100)')
    plt.plot(range(len(outcome7_K)), outcome7_K / K, label='spider_FW')
    plt.plot(range(len(outcome8_K)), outcome8_K / K, label='hybrid_spider_FW(100)')
    plt.plot(range(len(outcome11_K)), outcome11_K / K, label='hybrid_spider_FW(10)')
    plt.plot(range(len(outcome9_K)), outcome9_K / K, label='non_monotone_fw1')
    plt.plot(range(len(outcome10_K)), outcome10_K / K, label='non_monotone_fw2')
    # plt.plot(range(len(outcome12_K)), outcome12_K / K, label='non_monotone_fw3')
    plt.legend()
    plt.savefig("D:/2023文件/实验图像/20230702/non-monotone-qp/temp{average}.png")  # 输入地址，并利用format函数修改图片名称